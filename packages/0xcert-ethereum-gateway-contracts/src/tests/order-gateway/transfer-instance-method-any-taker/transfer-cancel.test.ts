...
spec.test('succeeds', async (ctx) => {
  ...
  await ctx.reverts(() => orderGateway.instance.methods.performAnyTaker(dataTuple, signatureTuple).send({ from: bob }), '015007');
});
...
spec.test('throws when trying to cancel an already performed atomic swap', async (ctx) => {
  ...
  await orderGateway.instance.methods.performAnyTaker(dataTuple, signatureTuple).send({ from: bob });
