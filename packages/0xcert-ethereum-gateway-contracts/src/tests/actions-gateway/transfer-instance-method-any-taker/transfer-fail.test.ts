...
spec.test('when proxy not allowed to transfer nft', async (ctx) => {
  ...
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '006004');
});
...
spec.test('when proxy has unsofficient allowence for a token', async (ctx) => {
  ...
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '001002');
});
...
spec.test('when current time is after expirationTimestamp', async (ctx) => {
  ...
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '015005');
});
...
spec.test('when signature is invalid', async (ctx) => {
  ...
  await cat.instance.methods.approve(nftSafeProxy.receipt._address, 1).send({ from: jane });
  await cat.instance.methods.approve(nftSafeProxy.receipt._address, 2).send({ from: bob });
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '015006');
});
...
spec.test('when trying to perform an already perfomed swap', async (ctx) => {
  ...
  await cat.instance.methods.approve(nftSafeProxy.receipt._address, 1).send({ from: jane });
  await cat.instance.methods.approve(nftSafeProxy.receipt._address, 2).send({ from: bob });
  await actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob });
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '015008');
});
...
spec.test('when trying to transfer third party assets', async (ctx) => {
  ...
  await cat.instance.methods.approve(nftSafeProxy.receipt._address, 2).send({ from: bob });
  await ctx.reverts(() => actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob }), '015004');
});
