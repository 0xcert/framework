...
spec.beforeEach(async (ctx) => {
  const bee = await ctx.deploy({
    src: '@0xcert/ethereum-erc721-contracts/build/nf-token-metadata-enumerable-mock.json',
    contract: 'NFTokenMetadataEnumerableMock',
    args: ['bee', 'BEE', 'https://0xcert.org/', '.json'],
  });
...
spec.test('Cat #1 <=> Cat #2', async (ctx) => {
  ...
  const logs = await actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob });
...
spec.test('Cat #1, Dog #1 <=> Fox #1, Bee #3', async (ctx) => {
  ...
  const logs = await actionsGateway.instance.methods.performAnyTaker(orderDataTuple, signatureDataTuple).send({ from: bob, gas: 6000000 });
  ctx.not(logs.events.Perform, undefined);
