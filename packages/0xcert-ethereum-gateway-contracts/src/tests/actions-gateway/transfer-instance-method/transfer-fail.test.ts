...
spec.test('when _to address is not the one performing the transfer', async (ctx) => {
  const actionsGateway = ctx.get('actionsGateway');
  const nftSafeProxy = ctx.get('nftSafeProxy');
  const jane = ctx.get('jane');
  ...
  await ctx.reverts(() => actionsGateway.instance.methods.perform(orderDataTuple, signatureDataTuple).send({ from: sara }), '015003');
});
