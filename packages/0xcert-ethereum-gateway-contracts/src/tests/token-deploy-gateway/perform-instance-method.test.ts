...
spec.test('fails when not specified taker', async (ctx) => {
  ...
  await zxc.instance.methods.approve(tokenProxy.receipt._address, 10000).send({ from: jane });
  await ctx.reverts(() => tokenDeployGateway.instance.methods.perform(createTuple, signatureDataTuple).send({ from: sara }), '009002');
});
...
spec.test('fails with expired claim', async (ctx) => {
  ...
  await zxc.instance.methods.approve(tokenProxy.receipt._address, 10000).send({ from: jane });
  await ctx.reverts(() => tokenDeployGateway.instance.methods.perform(createTuple, signatureDataTuple).send({ from: owner }), '009003');
});
...
spec.test('fails with invalid signature', async (ctx) => {
  ...
  await zxc.instance.methods.approve(tokenProxy.receipt._address, 10000).send({ from: jane });
  await ctx.reverts(() => tokenDeployGateway.instance.methods.perform(createTuple, signatureDataTuple).send({ from: owner }), '009004');
});
...
spec.test('fails trying to perform an already performed deploy', async (ctx) => {
  ...
  await ctx.reverts(() => tokenDeployGateway.instance.methods.perform(createTuple, signatureDataTuple).send({ from: owner }), '009006');
});
...
spec.test('fails trying to perform a canceled deploy', async (ctx) => {
  ...
  await ctx.reverts(() => tokenDeployGateway.instance.methods.perform(createTuple, signatureDataTuple).send({ from: owner }), '009005');
});
