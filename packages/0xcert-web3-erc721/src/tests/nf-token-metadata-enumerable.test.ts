...
spec.test('throws when trying to mint 2 NFTs with the same ids', async (ctx) => {
  ...
  const url1 = ctx.get('url1');
  await nftoken.instance.methods.mint(bob, id1, url1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.mint(bob, id1, url1).send({ from: owner }), '006006');
});
...
spec.test('throws when trying to mint NFT to 0x0 address', async (ctx) => {
  ...
  const id1 = ctx.get('id1');
  const url1 = ctx.get('url1');
  await ctx.reverts(() => nftoken.instance.methods.mint(zeroAddress, id1, url1).send({ from: owner }), '006001');
});
...
spec.test('throws when trying to burn non existant NFT', async (ctx) => {
  ...
  const owner = ctx.get('owner');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.burn(id1).send({ from: owner }), '006002');
});
...
spec.test('returns the correct token by index', async (ctx) => {
  ...
  await nftoken.instance.methods.mint(bob, id1, url1).send({ from: owner });
  await nftoken.instance.methods.mint(bob, id2, url2).send({ from: owner });
  await nftoken.instance.methods.mint(sara, id3, url3).send({ from: owner });
...
spec.test('mint should correctly set ownerToIds and idToOwnerIndex and idToIndex', async (ctx) => {
  ...
  await nftoken.instance.methods.mint(bob, id1, url1).send({ from: owner });
  await nftoken.instance.methods.mint(bob, id3, url3).send({ from: owner });
  await nftoken.instance.methods.mint(bob, id2, url2).send({ from: owner });
