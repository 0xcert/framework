...
spec.test('multiplies correctly', async (ctx) => {
  ...
  const a = 5678;
  const b = 1234;
  await safeMath.methods.multiply(a, b).send();
  const result = await safeMath.methods.result().call();
...
spec.test('adds correctly', async (ctx) => {
  ...
  const a = 5678;
  const b = 1234;
  await safeMath.methods.add(a, b).send();
  const result = await safeMath.methods.result().call();
  ctx.is(result, (a + b).toString());
});
...
spec.test('subtracts correctly', async (ctx) => {
  ...
  const a = 5678;
  const b = 1234;
  await safeMath.methods.subtract(a, b).send();
  const result = await safeMath.methods.result().call();
  ctx.is(result, (a - b).toString());
});
...
spec.test('should throw an error if subtraction result would be negative', async (ctx) => {
  ...
  const a = 1234;
  const b = 5678;
  await ctx.throws(() => safeMath.methods.subtract(a, b).send());
});
...
spec.test('should throw an error on addition overflow', async (ctx) => {
  ...
  const b = 1;
  await ctx.throws(() => safeMath.methods.add(a, b).send());
});
...
spec.test('should throw an error on multiplication overflow', async (ctx) => {
  ...
  const b = 2;
  await ctx.throws(() => safeMath.methods.multiply(a, b).send());
});