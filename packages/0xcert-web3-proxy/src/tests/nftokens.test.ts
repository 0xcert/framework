...
spec.test('fails when trying to add an already authorized address', async (ctx) => {
  ...
  await ctx.reverts(() => nftProxy.methods.addAuthorizedAddress(bob).send({ from: owner }), '013001');
});
...
spec.test('fails when trying to remove an already unauthorized address', async (ctx) => {
  ...
  const owner = ctx.get('owner');
  const bob = ctx.get('bob');
  await ctx.reverts(() => nftProxy.methods.removeAuthorizedAddress(bob).send({ from: owner }), '013002');
});
