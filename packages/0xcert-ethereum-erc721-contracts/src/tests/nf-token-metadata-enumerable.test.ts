...
spec.test('correctly checks all the supported interfaces', async (ctx) => {
  ...
  const nftokenMetadataEnumerableInterface = await nftoken.instance.methods.supportsInterface('0x780e9d63').call();
  const nftokenNonExistingInterface = await nftoken.instance.methods.supportsInterface('0xa40e9c95').call();
...
spec.test('throws when trying to get count of NFTs owned by 0x0 address', async (ctx) => {
  ...
  await ctx.reverts(() => nftoken.instance.methods.balanceOf(zeroAddress).call(), '006001');
});
...
spec.test('throws when trying to create 2 NFTs with the same ids', async (ctx) => {
  ...
  const id1 = ctx.get('id1');
  await nftoken.instance.methods.create(bob, id1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.create(bob, id1).send({ from: owner }), '006006');
});
...
spec.test('throws when trying to create NFT to 0x0 address', async (ctx) => {
  ...
  const zeroAddress = ctx.get('zeroAddress');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.create(zeroAddress, id1).send({ from: owner }), '006001');
});
...
spec.test('throws when trying to find owner od non-existing NFT id', async (ctx) => {
  const nftoken = ctx.get('nfToken');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.ownerOf(id1).call(), '006002');
});
...
spec.test('throws when trying to get approval of non-existing NFT id', async (ctx) => {
  const nftoken = ctx.get('nfToken');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.getApproved(id1).call(), '006002');
});
...
spec.test('throws when trying to approve NFT ID from a third party', async (ctx) => {
  ...
  await nftoken.instance.methods.create(bob, id1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.approve(sara, id1).send({ from: sara }), '006003');
});
...
spec.test('throws when trying to transfer NFT as an address that is not owner, approved or operator', async (ctx) => {
  ...
  await nftoken.instance.methods.create(bob, id1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.transferFrom(bob, jane, id1).send({ from: sara }), '006004');
});
...
spec.test('throws when trying to transfer NFT to a zero address', async (ctx) => {
  ...
  await nftoken.instance.methods.create(bob, id1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.transferFrom(bob, zeroAddress, id1).send({ from: bob }), '006001');
});
...
spec.test('throws when trying to transfer a invalid NFT', async (ctx) => {
  ...
  await nftoken.instance.methods.create(bob, id1).send({ from: owner });
  await ctx.reverts(() => nftoken.instance.methods.transferFrom(bob, sara, id2).send({ from: bob }), '006002');
});
...
spec.test('succesfully changes URI postfix', async (ctx) => {
  ...
  await nftoken.instance.methods.setUriPostfix(newPostfix).send({ from: owner });
  uri = await nftoken.instance.methods.tokenURI(id1).call();
...
spec.test('return empty string if URI base is empty', async (ctx) => {
  const nftoken = ctx.get('nfToken');
  const owner = ctx.get('owner');
  const bob = ctx.get('bob');
  const id1 = ctx.get('id1');
...
spec.test('throws when trying to get URI of invalid NFT ID', async (ctx) => {
  const nftoken = ctx.get('nfToken');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.tokenURI(id1).call(), '006002');
});
...
spec.test('corectly destroys a NFT', async (ctx) => {
  ...
  await ctx.reverts(() => nftoken.instance.methods.tokenByIndex(1).call(), '006007');
  await ctx.reverts(() => nftoken.instance.methods.tokenOfOwnerByIndex(bob, 1).call(), '006007');
});
...
spec.test('throws when trying to destroy non existant NFT', async (ctx) => {
  ...
  const owner = ctx.get('owner');
  const id1 = ctx.get('id1');
  await ctx.reverts(() => nftoken.instance.methods.destroy(id1).send({ from: owner }), '006002');
});
