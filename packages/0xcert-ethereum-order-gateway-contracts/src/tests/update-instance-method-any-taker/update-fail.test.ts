...
spec.test('throws when maker does not have ability to sign an update asset', async (ctx) => {
  ...
  await ctx.reverts(() => orderGateway.instance.methods.performAnyTaker(createTuple, signatureDataTuple).send({ from: bob }), '015010');
});
...
spec.test('throws when proxy does not have ability to update assets', async (ctx) => {
  ...
  await ctx.reverts(() => orderGateway.instance.methods.performAnyTaker(createTuple, signatureDataTuple).send({ from: jane }), '017001');
});
...
spec.test('throws when trying to perform and already performed update', async (ctx) => {
  ...
  await cat.instance.methods.grantAbilities(updateProxy.receipt._address, XcertAbilities.UPDATE_ASSET_IMPRINT).send({ from: owner });
  const logs = await orderGateway.instance.methods.performAnyTaker(createTuple, signatureDataTuple).send({ from: jane });
  ctx.not(logs.events.Perform, undefined);

  await ctx.reverts(() => orderGateway.instance.methods.performAnyTaker(createTuple, signatureDataTuple).send({ from: jane }), '015008');
});
...
spec.test('when current time is after expirationTimestamp', async (ctx) => {
  ...
  await ctx.reverts(() => orderGateway.instance.methods.performAnyTaker(createTuple, signatureDataTuple).send({ from: jane }), '015005');
});
