...
* **Unified experience for decentralized and distributed systems:** The goal of the 0xcert Framework is to become blockchain/platform-agnostic, without discriminating among different blockchains or other distributed systems. The entire API of the Framework is identical when used with the Ethereum blockchain or any other distributed system the developers choose for their applications. The code of API is identical and has the exact same functionality regardless of the chosen system they employ, enabling them to quickly and seamlessly switch between systems.
