...
The [ERC-721](https://github.com/ethereum/EIPs/blob/master/EIPS/eip-721.md) has become an important standard for describing non-fungible assets, not just in the Ethereum community but much broader. It has given us rules and directions for describing and managing unique assets. Although it represents an interoperability standard, there's a general miss conception about its overall purpose and about which part of the interoperability it covers. The ERC-721 specification states that the following standard allows for the implementation of a standard API for non-fungible tokens (NFTs) within smart contracts. It also states that it describes a basic functionality to track and transfer assets based on their IDs. So this standard covers the asset ownership aspect.
...
Asset schema is a specifically designed JSON object, which conforms to [RFC-7159](https://en.wikipedia.org/wiki/JSON) and follows the mapping format defined by the [JSON Schema](http://json-schema.org) specification. The 0xcert Protocol supports only a subset of JSON Schema specification which is described by the [Xcert JSON Schema](). You should always refer to this JSON Schema subset when writing asset schemas for your projects.

We incentivize the community to agree on standard schema conventions. Schema conventions should be proposed and included in the official 0xcert repository on Github. The 0xcert Framework already provides some basic [schema conventions]() which you can use. For the purpose of this guide, we will define a minimal custom asset schema which describes a context and properties of the metadata which we created earlier. 
...
Did you notice that a `description` property is not included in the schema? We did this intentionally just to show that the metadata object can include arbitrary properties. The schema, however, should include only the properties which are necessary to be included in the certification process. The properties defined in the schema are verifiable by a third-party. Other arbitrary metadata properties are not included in the certification process and are simply ignored.
...
The final artifact of the certification process is the `schema ID` string. It is calculated through the `Schema Hashing algorithm` (SHA) and uniquely represents the schema object. This ID must be stored as part of the immutable storage and identifies storage context (e.g. Ethereum smart contract).
...
John now verifies that the calculated imprint equals the imprint stored in the asset with ID `12` stored in the asset ledger owned by the Troopers Game Studio. If the imprint is a match it proves that we are telling the truth. He can now go ahead and perform the purchase because he was able to mathematically prove that the asset `12` really is the magic sword he wants. 

Of course, in reality, John would use a dapp where the whole process would be automated and nothing would have to be done by hand. Dapps would also provide other features like issuing and updating such certificates, burning and revoking of certificates, tracking and logging and more.
